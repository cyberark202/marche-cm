...
class _KycVerificationPageState extends State<KycVerificationPage> {
  ...
  String? _safePath(PlatformFile f) {
    try {
      final p = f.path;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() {
        _uploading = false;
        _errorMessage =
            _api.toUserMessage(e, fallback: 'Erreur lors de l\'envoi. Réessayez.');
      });
    }
  }
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: FilledButton.icon(
          ...
          icon: _uploading ? const SizedBox.shrink() : Icon(icon, size: 18),
          label: _uploading
              ? const SizedBox(
                  width: 20,
                  height: 20,
...
class _DocUploadCard extends StatelessWidget {
  ...
  final String? Function(PlatformFile) safePath;
  ...
  Widget build(BuildContext context) {
    ...
    final path = file != null ? safePath(file!) : null;
