    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Trade arrows (fusion logo concept: Afrique ↔ Chine)
          CustomPaint(
            size: const Size(140, 140),
            painter: _TradeArrowsPainter(progress: flagProgress),
          ),
          // Inner white badge with CM flag gradient
          Container(
            width: 100,
            height: 100,
            decoration: BoxDecoration(
          ...
          // Cameroon star badge (replaces lightning bolt)
          Positioned(
            top: 0,
            right: 0,
...
class _TradeArrowsPainter extends CustomPainter {
  const _TradeArrowsPainter({required this.progress});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Arc 1: upper half — 210° → 330° (over top) — green (Africa side)
    _drawArcArrow(
      canvas, cx, cy, r,
      startAngle: 7 * math.pi / 6,
      sweepAngle: 2 * math.pi / 3,
      color: AppPalette.cmGreen.withValues(alpha: 0.72 * opacity),
    );

    // Arc 2: lower half — 30° → 150° (under bottom) — red (exchange side)
    _drawArcArrow(
      canvas, cx, cy, r,
      startAngle: math.pi / 6,
      sweepAngle: 2 * math.pi / 3,
      color: AppPalette.cmRed.withValues(alpha: 0.60 * opacity),
    );
  }
  ...
  void _drawArcArrow(
    ...
    // Draw arc slightly short so arrowhead doesn't overlap the stroke end
    canvas.drawArc(rect, startAngle, sweepAngle - 0.18, false, paint);
    ...
    // Tangent for clockwise motion: d/dθ (cos θ, sin θ) = (-sin θ, cos θ)
    final tdx = -math.sin(endAngle);
    final tdy = math.cos(endAngle);
    ...
    final arrowPath = Path()
      ..moveTo(tipX + tdx * arrowLen, tipY + tdy * arrowLen)
      ..lineTo(tipX + pdx * arrowWidth, tipY + pdy * arrowWidth)
      ..lineTo(tipX - pdx * arrowWidth, tipY - pdy * arrowWidth)
      ..close();
  ...
  @override
  bool shouldRepaint(_TradeArrowsPainter old) => old.progress != progress;
}
