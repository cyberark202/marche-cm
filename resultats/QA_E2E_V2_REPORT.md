...
### 4.4 🔴 CRITIQUE — `NOTCHPAY_PUBLIC_KEY` vide-mais-présent casse tous les top-ups en LIVE
- **Endpoint :** `POST /api/wallets/topup/`.
- **Repro :** mode LIVE avec `NOTCHPAY_PUBLIC_KEY=` (ligne présente mais vide) dans l'env.
- **Observé :** `502 {"detail":"Echec initialisation paiement."}`, log `notchpay_checkout_error provider_error=Configuration NotchPay incomplete.`
- **Cause :** `_env_str_alias` (settings.py:504) ne retombe sur `NOTCHPAY_LIVE_PUBLIC_KEY` que si `NOTCHPAY_PUBLIC_KEY` est **ABSENT** ; une ligne vide renvoie `""` → `is_configured()` = False ([notchpay_checkout_service.py:16](backend/apps/wallets/notchpay_checkout_service.py#L16)).
- **Impact business :** **aucune recharge possible** en production → blocage total des entrées d'argent.
- **Correctif :** lignes `NOTCHPAY_PUBLIC_KEY=`/`NOTCHPAY_PRIVATE_KEY=` commentées dans `marche-cm.env` (fallback LIVE rétabli). **À répliquer dans le dashboard Render** (supprimer ces vars vides).

### 4.5 🟠 MAJEUR — `NOTCHPAY_CHECKOUT_RETURN_URL` vide → redirection navigateur vers le webhook (405)
- **Observé :** après paiement, le navigateur est redirigé en **GET** sur `/api/wallets/notchpay/checkout/webhook/` → `405 Method Not Allowed` (au lieu d'une page de succès). Cause : `RETURN_URL` vide → fallback sur `CALLBACK_URL` (= URL du webhook), settings ~l.125.
- **Impact :** l'acheteur tombe sur une erreur après avoir payé. **Correctif :** définir `NOTCHPAY_CHECKOUT_RETURN_URL` vers une page de succès front.

### ✅ Paiement LIVE validé bout-en-bout (avec consentement du propriétaire)
- Top-up **500 FCFA réel** : lien généré → **débit Mobile Money MTN approuvé** → NotchPay `status=complete` (sandbox=0) → **réconciliation** (`reconcile_pending_transactions`) → **wallet crédité +500 (82 800 → 83 300)** + **écriture ledger double-entrée**. Transaction expirée (lien non payé) correctement marquée `FAILED`.
- ⚠️ **Note opérationnelle :** le webhook serveur→serveur de NotchPay vise l'URL du **dashboard** (= prod `onrender.com`). Backend déployé DOWN → le webhook n'arrive pas ; c'est la **commande de réconciliation** qui a crédité (fallback fonctionnel, à planifier en Celery beat — déjà prévu).
...
- **NotchPay LIVE :** **un** débit réel de 500 FCFA a été effectué avec le consentement du propriétaire pour valider le dépôt bout-en-bout (cf. §4.5 ✅). Les autres chemins paiement s'arrêtent avant `create_invoice`/`send_money`. Le décaissement (retrait/payout, `send_money`) n'a pas été déclenché.
